...
// spacing and container alignment as EducationRow/AIToolsRow.
const ENTERTAINMENT_IDS = ["netflix", "jiohotstar", "amazon-prime-video", "sonyliv", "zee5", "apple-tv"];

export function EntertainmentRow() {
  const select = useUniverseStore((s) => s.select);
  const items = ENTERTAINMENT_IDS.map((id) => SUBSCRIPTIONS_BY_ID[id]).filter((s) => s !== undefined);
  if (items.length === 0) return null;

  return (
    <div className="px-4 pb-2 pt-5 lg:px-8">
      <div className="mx-auto max-w-[1340px]">
        <h2 className="mb-3 text-lg font-semibold text-ink-0">Entertainment</h2>
