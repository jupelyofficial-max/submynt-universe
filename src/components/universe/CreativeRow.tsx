...
const CREATIVE_IDS = ["adobe-creative-cloud", "canva-pro", "capcut-pro", "figma-professional", "envato-elements", "freepik"];

export function CreativeRow() {
  const select = useUniverseStore((s) => s.select);
  const items = CREATIVE_IDS.map((id) => SUBSCRIPTIONS_BY_ID[id]).filter((s) => s !== undefined);
  if (items.length === 0) return null;

  return (
    <div className="px-4 pb-2 pt-5 lg:px-8">
      <div className="mx-auto max-w-[1340px]">
        <h2 className="mb-3 text-lg font-semibold text-ink-0">Creative</h2>
