...
const TELECOM_IDS = ["jio", "airtel", "vi", "bsnl", "act-fibernet", "tata-play-fiber"];

export function TelecomRow() {
  const select = useUniverseStore((s) => s.select);
  const items = TELECOM_IDS.map((id) => SUBSCRIPTIONS_BY_ID[id]).filter((s) => s !== undefined);
  if (items.length === 0) return null;

  return (
    <div className="px-4 pb-2 pt-5 lg:px-8">
      <div className="mx-auto max-w-[1340px]">
        <h2 className="mb-3 text-lg font-semibold text-ink-0">Telecom & Internet</h2>
