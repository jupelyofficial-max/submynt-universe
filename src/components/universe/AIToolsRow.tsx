...
// spacing and container alignment as EducationRow.
const AI_TOOLS_IDS = ["chatgpt-plus", "claude-pro", "google-ai-pro", "perplexity-pro", "supergrok", "microsoft-copilot-pro"];

export function AIToolsRow() {
  const select = useUniverseStore((s) => s.select);
  const items = AI_TOOLS_IDS.map((id) => SUBSCRIPTIONS_BY_ID[id]).filter((s) => s !== undefined);
  if (items.length === 0) return null;

  return (
    <div className="px-4 pb-2 pt-5 lg:px-8">
      <div className="mx-auto max-w-[1340px]">
        <h2 className="mb-3 text-lg font-semibold text-ink-0">AI Tools</h2>
