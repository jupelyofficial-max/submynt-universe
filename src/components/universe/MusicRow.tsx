...
const MUSIC_IDS = ["spotify-premium", "youtube-music-premium", "apple-music", "amazon-music-unlimited", "jiosaavn-pro", "gaana-plus"];

export function MusicRow() {
  const select = useUniverseStore((s) => s.select);
  const items = MUSIC_IDS.map((id) => SUBSCRIPTIONS_BY_ID[id]).filter((s) => s !== undefined);
  if (items.length === 0) return null;

  return (
    <div className="px-4 pb-2 pt-5 lg:px-8">
      <div className="mx-auto max-w-[1340px]">
        <h2 className="mb-3 text-lg font-semibold text-ink-0">Music</h2>
