...
/** Personalized picks across the whole catalogue — reuses the same
 * getRecommendation scoring engine DetailPanel's single "Recommended for
 * you" card already uses, just applied to every not-yet-owned subscription
 * instead of one. Driven by useMySubscriptionsStore's local `owned` list,
 * same as everywhere else "owned" is read in this app — not gated on being
 * signed in, even though the only nav entry point to this page
 * (AccountMenu) is. */
export default function ForYouPage() {
  ...
  const results = useMemo(() => {
    ...
    const candidates = SUBSCRIPTIONS.filter((s) => !ownedIds.has(s.id));
    ...
    <div className="mx-auto w-full max-w-5xl flex-1 px-4 py-10 lg:px-8">
      ...
      {results.length === 0 ? (
        <div className="glass-panel flex flex-col items-center gap-3 rounded-2xl p-16 text-center">
          ...
          <p className="max-w-sm text-sm text-ink-400">Check back once there&apos;s more here to compare against.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {results.map((result) => (
            <RecommendationCard
              ...
              result={result}
              onExplore={() => openDetails(result.subscription.id)}
