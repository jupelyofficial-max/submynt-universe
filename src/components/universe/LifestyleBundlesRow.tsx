  ...
  return (
    <div className="px-4 pb-2 pt-5 lg:px-8">
      <div className="mx-auto max-w-[92rem]">
        ...
        <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto no-scrollbar pb-1">
          {CARDS.map((card) => (
            <Link
              key={card.key}
              href={card.href}
              aria-label={card.alt}
              className="relative aspect-[3/1] w-[85%] shrink-0 snap-start overflow-hidden rounded-3xl transition-transform duration-200 hover:scale-[1.01] sm:w-[55%] lg:w-[38%]"
