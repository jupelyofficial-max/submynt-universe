...
export function ViewSwitcher() {
  ...
  return (
    ...
    <div className="flex h-9 items-center rounded-full bg-[#22c55e] p-1 gap-1">
      {/* White text on this green measures ~2.0-2.3:1 (WCAG AA needs
          4.5:1) — ink-0 on the same green is ~7.9-8.9:1. */}
      <button
        onClick={() => setViewMode("universe")}
        className={cn(
          "rounded-full px-3.5 h-full text-xs font-semibold transition-colors cursor-pointer",
          viewMode === "universe" ? "bg-white/15 text-ink-0" : "text-ink-0/80 hover:text-ink-0"
        )}
      >
        Universe
      </button>
      <button
        ...
        className={cn(
          "rounded-full px-3.5 h-full text-xs font-semibold transition-colors cursor-pointer",
          viewMode === "list" ? "bg-white/15 text-ink-0" : "text-ink-0/80 hover:text-ink-0"
        )}
      >
        List
      </button>
