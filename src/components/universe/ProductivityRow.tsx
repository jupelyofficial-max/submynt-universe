...
// Real catalog ids only (src/data/subscriptions.ts) — same card markup,
// spacing and container alignment as EducationRow/AIToolsRow/MusicRow.
const PRODUCTIVITY_IDS = ["notion-plus", "microsoft-365", "google-workspace", "todoist-pro", "evernote-starter", "dropbox-plus"];

export function ProductivityRow() {
  const select = useUniverseStore((s) => s.select);
  const items = PRODUCTIVITY_IDS.map((id) => SUBSCRIPTIONS_BY_ID[id]).filter((s) => s !== undefined);
  if (items.length === 0) return null;

  return (
    <div className="px-4 pb-2 pt-5 lg:px-8">
      <div className="mx-auto max-w-[1340px]">
        <h2 className="mb-3 text-lg font-semibold text-ink-0">Productivity</h2>
            ...
            return (
              ...
              >
                {/* Fixed neutral backing, logo inset within it — normalizes
                    ...
                    ratio varies, since every card anchors to the same
                    backing footprint rather than each raw image's own
                    bounding box (see EducationRow, same treatment). */}
